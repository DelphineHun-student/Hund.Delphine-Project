...
Open the files 
```{r}
file_rdata <- file.path(RES_PATH, "pre_manip_ratings.rdata")
load(file_rdata)
```
...
ratings_clean <- ratings_clean %>%
  mutate(
    morpho_target = case_when(
      str_detect(avatar, "_ATH_")    ~ "athletique",
      str_detect(avatar, "_AVG_")    ~ "moyen",
      str_detect(avatar, "_OW_")     ~ "surpoids",
      str_detect(avatar, "_OBESE_")  ~ "obesite",
      str_detect(avatar, "_THIN_")   ~ "fine",
      str_detect(avatar, "_SKINNY_") ~ "maigre",
      TRUE ~ NA_character_
    ),
    morpho_target = factor(morpho_target,
      levels = c("athletique","moyen","surpoids","obesite","fine","maigre")
    ),
